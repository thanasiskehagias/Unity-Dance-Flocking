...
public class DroneBehavior : MonoBehaviour {
	...
	public DroneParams jsScript; 
	
	void Awake()
	{
		Drones=GameObject.Find("Drones");
		jsScript = Drones.GetComponent<DroneParams>();
	}
	
	void FixedUpdate()
	{
		//print(jsScript.TIME.ToString());
		//GUI.Label(new Rect(10,10,300,20),jsScript.name);
		// we should always apply physics forces in FixedUpdate
		if( Input.GetKeyDown( KeyCode.Z )) AutoMode=true;
		if( Input.GetKeyDown( KeyCode.X )) AutoMode=false;
		Flock(AutoMode);
	}
	...
	protected virtual void Start()
	{
		...
		SepSlider=GameObject.Find("SepSlider").GetComponent<Slider>();
		AliSlider=GameObject.Find("AliSlider").GetComponent<Slider>();
		CohSlider=GameObject.Find("CohSlider").GetComponent<Slider>();
	}
	...
	public virtual void Flock(bool Automode)
	{
		...
		/*
		if( Input.GetKeyDown( KeyCode.Q ) ) separationWeight+=0.5f;
		if( Input.GetKeyDown( KeyCode.A ) ) separationWeight-=0.5f;
		if( Input.GetKeyDown( KeyCode.W ) ) alignmentWeight+=0.5f;
		if( Input.GetKeyDown( KeyCode.S ) ) alignmentWeight-=0.5f;
		if( Input.GetKeyDown( KeyCode.E ) ) cohesionWeight+=0.5f;
		if( Input.GetKeyDown( KeyCode.D ) ) cohesionWeight-=0.5f;
		*/
		
		if(!AutoMode)
		{
			maxSpeed=SpeedSlider.value;
			neighborRadius=RadiusSlider.value;
			separationWeight=SepSlider.value;
			alignmentWeight=AliSlider.value;
			cohesionWeight=CohSlider.value;
		}
		if(AutoMode)
		{
			neighborRadius  =jsScript.neighborRadius;	//.100.0f*(3.0f+Mathf.Sin(0.200f*Time.time));
			separationWeight=jsScript.separationWeight; //2.50f*(1.0f+Mathf.Sin(0.120f*Time.time));
			alignmentWeight =jsScript.alignmentWeight; 	// 2.50f*(1.0f+Mathf.Sin(0.160f*Time.time));
			cohesionWeight  =jsScript.cohesionWeight; 	//4.20f*(0.0f+Mathf.Sin(0.240f*Time.time)+0.20f);
			RadiusSlider.value=neighborRadius;
			SepSlider.value=separationWeight;
			AliSlider.value=alignmentWeight;
			CohSlider.value=cohesionWeight;
		}
